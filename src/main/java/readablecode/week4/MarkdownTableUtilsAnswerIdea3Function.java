...
public class MarkdownTableUtilsAnswerIdea3Function {
	...
	private static String createSeparatorRow(List<String> headerRowCaptions) {
		return createRow(createConvertedCaptions(headerRowCaptions, caption -> Strings.repeat(HYPHEN, caption.length())));
	}
	...
	private static String createEmptyRow(List<String> headerRowCaptions) {
		return createRow(createConvertedCaptions(headerRowCaptions, caption -> Strings.repeat(SPACE, caption.length())));
	}

	private static List<String> createConvertedCaptions(List<String> captions, Function<String, String> converter) {
		return captions.stream().map(converter).collect(Collectors.toList());
	}
